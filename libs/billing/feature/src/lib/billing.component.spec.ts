...
const mockSubscription = (overrides: Partial<SubscriptionResponse> = {}): SubscriptionResponse => ({
...
describe('BillingComponent', () => {
  ...
  function createBillingApiMock() {
    return {
      ...
      createCheckoutSession: vi.fn(() => of({ url: 'https://checkout.stripe.com/session' })),
      createPortalSession: vi.fn(() => of({ url: 'https://billing.stripe.com/portal' })),
      cancelSubscription: vi.fn(() => of({ message: 'Subscription cancelled' })),
  ...
  describe('ngOnInit', () => {
    ...
    it('sets error on API failure', async () => {
      billingApiMock.getSubscription = vi.fn(() => throwError(() => new Error('Failed')));
      component = TestBed.runInInjectionContext(() => new BillingComponent());
  ...
  describe('upgrade', () => {
    ...
    it('shows error message on failure', async () => {
      billingApiMock.createCheckoutSession = vi.fn(() => throwError(() => new Error('Failed')));
      component = TestBed.runInInjectionContext(() => new BillingComponent());
  ...
  describe('openPortal', () => {
    ...
    it('shows error message on failure', async () => {
      billingApiMock.createPortalSession = vi.fn(() => throwError(() => new Error('Failed')));
      component = TestBed.runInInjectionContext(() => new BillingComponent());
  ...
  describe('confirmCancel', () => {
    ...
    it('cancels subscription when confirmed', async () => {
      ...
      expect(billingApiMock.cancelSubscription).toHaveBeenCalledWith({ orgId: 'org-1' });
    });
