...
- 🛠️ **Admin backoffice portal** — super-admin area (protected by `isSystemAdmin` flag) with org list, org detail, member management, billing overview with Stripe portal access, per-org and cross-tenant activity log, per-org entitlement override management (set / delete with reason, expiry date, and `createdByName` column), entitlement cache invalidation
...
apps/
  ...
  admin/          — Admin MFE (port 4203): super-admin backoffice portal (org inspection, members, billing, activity log, entitlement override management)

libs/
  admin/
    data-access/  — AdminApi (org list, org detail, members, billing, activity log, entitlements)
  auth/
...
| ------------------------------------------ | --------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `@saas-frontend/admin/data-access`         | [→](libs/admin/data-access/README.md)         | `AdminApi` — org list/detail, members, billing, activity log, entitlement override CRUD (admin only) |
| `@saas-frontend/shared/util-rbac`          | [→](libs/shared/util-rbac/README.md)          | `PermissionsService`, `*hasPermission`, `*hasPlan`, `*hasEntitlement` directives                     |
